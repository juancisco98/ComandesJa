...
import { MOCK_PRODUCTS, MOCK_CUSTOMERS } from '../services/mockService';
...
import { useData } from '../context/DataContext'; // Import global context
...
const MarketingView: React.FC = () => {
  const { user } = useAuth();
  const { coupons, addCoupon, updateCoupon, deleteCoupon } = useData(); // Use Global Data
  ...
  // State
  // products are local for view, but featured toggle simulates local change for now or could be global
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.createOfferFor) {
        ...
        // Check if exists in context coupons
        const exists = coupons.find(c => c.code === generatedCode);
  ...
  const handleSendCampaign = () => {
      ...
      setTimeout(() => {
          ...
          // 1. Broadcast para Popup Instantáneo (Afecta a todos los clientes conectados)
          const broadcastData = {
              id: `BCAST-${Date.now()}`,
              title: campaignForm.title,
              message: campaignForm.message,
          ...
          // Simula envío masivo a la "base de datos" de notificaciones
          ...
          showNotification(`Difusión enviada a ${audienceCount} clientes registrados.`);
      }, 1500);
  };
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                        ...
                        <div className="divide-y divide-gray-100">
                            {coupons.map(coupon => (
                                <div 
                                    ...
                                    className={`p-4 flex items-center justify-between hover:bg-gray-50 cursor-pointer group transition-colors ${!coupon.isActive ? 'bg-gray-50 opacity-60' : ''}`}
                                >
                                    <div className="flex items-center">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 group-hover:bg-indigo-100 transition-colors ${coupon.isActive ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-200 text-gray-500'}`}>
                                            <Ticket size={18} />
                                        </div>
                                        <div>
                                            <p className="font-bold text-gray-800 flex items-center gap-2">
                                                {coupon.code}
                                                {!coupon.isActive && <span className="text-[10px] bg-red-100 text-red-600 px-1.5 rounded">AGOTADO</span>}
                ...
                <div className="xl:col-span-1">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-full flex flex-col">
                        ...
                        <div className="mb-4 bg-indigo-50 text-indigo-700 text-xs p-2 rounded border border-indigo-100">
                            📢 Este mensaje llegará a todos los clientes registrados como una notificación push.
                        </div>
                        <input 
                            type="text" 
                        ...
                        >
                            {isSending ? 'Enviando a Base de Datos...' : 'Enviar Notificación'}
                        </button>
                    </div>
                </div>
            </div>
