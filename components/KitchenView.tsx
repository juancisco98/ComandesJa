...
const KitchenView: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const unsubscribe = orderService.subscribeToOrders(user.id, (updatedOrders) => {
            setOrders(updatedOrders);
            // Play sound if new order added (logic can be improved to check count)
            playSound();
        });
    ...
    const handleProcessOrder = async (orderId: string) => {
        ...
        await orderService.updateOrderStatus(orderId, OrderStatus.COOKING);
        // La suscripción actualizará el estado local automáticamente
    ...
    const updateStatus = async (orderId: string, newStatus: OrderStatus, additionalFields?: Partial<Order>) => {
        // Para actualizaciones simples de estado
        await orderService.updateOrderStatus(orderId, newStatus);
        // Nota: additionalFields como readyAt no se estan guardando en DB en esta version simple de orderService,
        // pero el 'status' sí. Si necesitamos readyAt, deberíamos actualizar orderService.
    };
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="space-y-4">
                        {pendingOrders.map((order, idx) => (
                            <div key={order.id} className="bg-white rounded-xl shadow-lg border border-red-100 overflow-hidden relative animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
                                {/* Red Header Alert */}
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="mb-4">
                                        <span className={`inline-flex items-center text-xs font-bold px-2 py-1 rounded uppercase tracking-wide ${order.deliveryType === 'DELIVERY' ? 'bg-gray-100 text-gray-600' : 'bg-blue-100 text-blue-600'
                                            }`}>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mt-4">
                                        ...
                                        <button
                                            className="bg-emerald-500 hover:bg-emerald-600 text-white font-black uppercase py-3 rounded-lg shadow-md shadow-emerald-200 btn-press flex items-center justify-center"
                                        >
                        ...
                        {cookingOrders.map((order, idx) => (
                            <div key={order.id} className="bg-white rounded-xl shadow-sm border-2 border-orange-100 overflow-hidden relative">
                                <div className="p-5">
                                    ...
                                    <div className="mb-2">
                                        <span className={`inline-flex items-center text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide ${order.deliveryType === 'DELIVERY' ? 'bg-gray-100 text-gray-500' : 'bg-blue-50 text-blue-500'
                                            }`}>
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="space-y-4">
                        {readyOrders.map(order => {
                            ...
                            return (
                                <div key={order.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden relative transition-all duration-300 ${isStagnant
                                    ...
                                    <div className="p-5">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase ${isDelivery ? 'bg-gray-100 text-gray-600' : 'bg-emerald-500 text-white'
                                                    }`}>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`flex items-center justify-center space-x-1 py-2 rounded text-xs font-bold transition-all border ${isNotified || isOnWay
                                            ...
                                            <button
                                                ...
                                                className={`font-bold py-2 rounded text-xs transition-colors ${(!isNotified && !isOnWay)
