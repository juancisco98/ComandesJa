...
const CustomerHome: React.FC = () => {
    ...
    const [stores, setStores] = useState<any[]>([]); // Real stores state
        ...
        const fetchStores = async () => {
            ...
            try {
                const { data, error } = await supabase
                    .from('locales')
                    .select('*')
                    .eq('status', 'active'); // Only active stores

                if (data) {
                    const mappedStores = data.map((s: any) => ({
                        id: s.owner_id, // Use owner_id as link
                        name: s.nombre_local,
                        slug: s.nombre_local.toLowerCase().replace(/ /g, '-'), // Generate slug on fly if missing
                        category: s.categoria,
                        businessType: s.categoria, // Map 'Restaurante' -> 'Restaurante' etc.
                        rating: 4.5, // Default/Mock logic for now
                        deliveryFee: 1.50, // Default
                        deliveryTimeRange: '30-45 min',
                        minOrder: 10,
                        image: 'https://images.unsplash.com/photo-1514933651103-005eec06c04b?auto=format&fit=crop&q=80&w=1000', // Random placeholder or specific
                        tags: ['Nuevo'],
                        isOpen: true
                    }));
                    setStores(mappedStores);
                } else {
                    // Fallback to mock if DB empty or error to avoid blank screen
    ...
    const handleRepeatOrder = (order: Order) => {
        // Find store in the fetched 'stores' list (or MOCK fallback)
        const store = stores.find(s => s.id === order.tenantId) || MOCK_STORES.find(s => s.id === order.tenantId);
    ...
    const businessCategories = [
        ...
        { name: 'Bar', icon: <Coffee size={20} />, label: 'Bar' },
        { name: 'Cafetería', icon: <Coffee size={20} />, label: 'Café' },
    ...
    const filteredStores = stores.filter(store => {
        ...
        // Mock Logic for Promos
        const matchesPromos = showPromosOnly ? (store.rating >= 4.5) : true;
                                ...
                                <div className="px-4 pb-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                        {filteredStores.map((store, idx) => (
                                            <div key={store.id} onClick={() => navigate(`/store/${store.slug}`)} className="soft-card overflow-hidden cursor-pointer hover-lift group animate-fade-in" style={{ animationDelay: `${idx * 50}ms` }}>
