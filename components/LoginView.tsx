...
const LoginView: React.FC = () => {
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        // HARDCODED: Registration is always for CUSTOMER role via public interface
        const { success, error: regError } = await register(email, password, name, 'CUSTOMER');

        if (!success && regError) {
            setError(regError.message || 'Error al registrarse. Intentalo de nuevo.');
        } else if (!success) {
            setError('Error desconocido. Revisa tu conexión.');
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
            ...
            <button 
                onClick={() => login('cliente@demo.com', 'password')} 
