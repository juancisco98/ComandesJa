...
import { MOCK_STORES, MOCK_CONFIG, MOCK_ANALYTICS } from '../services/mockService';
...
const StoreFront: React.FC = () => {
    ...
    const { coupons, useCoupon } = useData();
    // Note: We will fetch products specific to this store locally to avoid mixing stores
    ...
    // Dynamic Data State
    const [store, setStore] = useState<any>(null); // Use explicit type if available
    ...
    useEffect(() => {
        const fetchStoreData = async () => {
            setIsLoading(true);
            try {
                // 1. Resolve Slug to Store (Naive approach: check ID or Name match)
                // In production, we should have a 'slug' column in 'locales'
                let { data: storeData, error } = await supabase
                    .from('locales')
                    .select('*')
                    .ilike('nombre_local', slug?.replace(/-/g, ' ') || '%')
                    .maybeSingle();

                // Fallback: If no match found by name, try to just get the first one for demo
                if (!storeData) {
                    const { data: firstStore } = await supabase.from('locales').select('*').limit(1).single();
                    storeData = firstStore;
                }

                if (storeData) {
                    // Map DB locale to StoreProfile UI type
                    const mappedStore = {
                        id: storeData.owner_id, // IMPORTANT: Use owner_id as identifier for orders
                        name: storeData.nombre_local,
                        slug: slug || 'default',
                        businessType: storeData.categoria as any,
                        ...
                        banner: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&q=80&w=2000',
                        isOpen: true,
                    ...
                    if (productsData) {
                        const mappedProducts: Product[] = productsData.map((p: any) => ({
                            ...
                            category: 'General', // We need to join categories in valid implementation
    ...
    const bestSellerIds = products.slice(0, 3).map(p => p.id); // Simple first 3 as bestsellers

    if (isLoading) return <div className="flex h-screen items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-emerald-600"></div></div>;
    ...
    const handleApplyCoupon = () => {
        ...
        // Look for coupon in GLOBAL context
        ...
        if (found) {
            ...
            alert("❌ Este cupón no es válido, ha expirado o se ha agotado.");
            setAppliedCoupon(null);
        }
    };
    ...
    const handlePlaceOrder = async () => {
        ...
        // (Mock config used for now, ideally fetch from store settings)
        const now = new Date();
        ...
        const orderData: Partial<any> = {
            tenantId: store.id, // owner_id from fetched store
            customerName: `Cliente Web (${phone})`, // Simple name for now
            total: finalTotal,
            items: cart.map(item => {
                const product = products.find(p => p.id === item.id);
                return {
            ...
            // Additional metadata could be added to 'notes' or specific fields
        };

        // 3. Submit to Supabase via Service
        const result = await orderService.createOrder(orderData, store.id);
        ...
        <div className={`min-h-screen bg-[#F8F9FA] pb-24 relative ${isTenantPreview ? 'pt-12' : ''}`}>
                    ...
                    <div className="space-y-8">
                        {uniqueCategories.map(category => (
                            <div key={category} id={`category-${category}`} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden scroll-mt-40">
                                ...
                                <div className="space-y-4 p-4 bg-gray-50/30">
                                    {products.filter(p => p.category === category).map((product, idx) => {
                                        ...
                                        return (
                                            <div
                                                key={product.id}
                                                className="p-4 flex gap-4 soft-card cursor-pointer group hover-lift animate-fade-in bg-white"
                    ...
                    <div className="sticky top-24 space-y-4">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
                            ...
                            <button
                                ...
                                className="w-full border border-gray-300 text-gray-700 font-bold py-2.5 rounded-xl text-sm hover:bg-gray-50 transition-colors btn-press"
                            >
                                Programar pedido
                            </button>
                        </div>
                            ...
                            {cart.length === 0 ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button
                                        ...
                                        className="w-full bg-emerald-600 text-white font-bold py-3.5 rounded-xl hover:bg-emerald-700 shadow-md hover:shadow-lg transition-all btn-press"
                                    >
                                        Ir a Pagar
                                    </button>
                                </div>
            ...
            {cart.length > 0 && (
                <div className="fixed bottom-6 left-0 right-0 px-4 z-40 lg:hidden">
                    <button
                        onClick={() => handleOpenCheckout('ASAP')}
                        className="max-w-3xl mx-auto w-full bg-emerald-600 text-white p-4 rounded-xl shadow-xl flex justify-between items-center hover:bg-emerald-700 transition-transform btn-press"
                    >
                        <div className="flex items-center space-x-2">
