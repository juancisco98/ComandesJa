        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          {plans.map((plan) => (
            <div key={plan.id} className={`relative ${plan.highlight ? 'md:-mt-4' : ''}`}>
              ...
              <ClayCard
                className={`
                  p-8 flex flex-col h-full relative transition-all duration-300 cursor-pointer
                  ${plan.highlight ? 'border-primary/50 shadow-glass-hover hover:scale-105' : 'border-white/60 hover:scale-102'}
                  hover:shadow-2xl
                `}
                hoverEffect={true}
              >
